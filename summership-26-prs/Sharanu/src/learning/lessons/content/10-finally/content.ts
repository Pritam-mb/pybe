...
const finallyClause: Lesson = {
  ...
  storyBody: `Arjuna's bow is still on the ground. Krishna hasn't convinced him to fight yet — he's still working through it, piece by piece. Now he says something that sounds like it might make things easier, and turns out to ask for more instead.

"You have a right to perform your duty," Krishna tells him, "but you are not entitled to the fruits of your actions." Not: fight, and you'll win. Not: fight, and it'll turn out well. He makes no promise about the outcome at all.

What he's asking is narrower and stranger — that Arjuna act because it's his duty to act, whether the result is victory, defeat, grief, or relief.

This isn't indifference to outcomes. It's a claim that duty doesn't wait around for a guarantee before it's owed. Whatever happens to Arjuna's brothers, his teachers, himself — the action has to happen regardless.`,
  ...
  conceptExplainer: `Python's try statement can include a finally clause — code that runs no matter what happens in the try block: whether it succeeds cleanly, raises an exception that gets caught, or even raises one that doesn't get caught at all.
...
Whether fight_battle() succeeds, raises a BattleLostError that gets handled, or raises something else entirely, the finally block still runs before the program moves on — or before the exception propagates further.`,

  mappingExplainer: `The whole force of Krishna's teaching in this verse is that the duty isn't conditional on the result. It doesn't fire only in the success case, and it doesn't get skipped just because things went badly — it's owed either way, unconditionally, as a separate commitment from whatever the outcome turns out to be.

finally is the one part of a try statement built with exactly that unconditional quality. Code in except only runs if something specific went wrong; code after the try block only runs if nothing propagated past it uncaught. finally doesn't check any of that — it runs regardless, the way Krishna's account of duty doesn't check the scoreboard before deciding whether it applies. It's usually where you put things that must happen either way: closing a file, releasing a resource, logging that an attempt was made at all — the code equivalent of an action performed because it's owed, not because of how things turned out.`,
...
attempt_action(fight_battle)`,
    ...
    {
      stage: "apply",
      prompt: "A database connection is opened at the start of a function and absolutely must be closed no matter what happens inside — even if the code between raises an exception no one catches. Where does connection.close() belong, and why is finally specifically the right tool here instead of just putting it at the end of the function?",
    },
  ],
