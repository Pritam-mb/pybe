...
const raiseKeyword: Lesson = {
  ...
  storyBody: `In the Kaurava court, Yudhishthira has wagered and lost everything — his wealth, his brothers, himself, and finally Draupadi. Dushasana drags her into the hall by her hair and tries to disrobe her in front of the assembled kings. Krishna intervenes and saves her from that horror.
...
The vow doesn't resolve anything in that moment. It sits there, unfulfilled, for years — through exile, through war — until Bhima finally fulfills it on the battlefield. Draupadi didn't wait for the story to produce that reckoning on its own. She raised the condition herself, explicitly.`,
  ...
  conceptExplainer: `Every exception so far in this module happened because Python's interpreter detected something invalid and raised it automatically — dividing by zero, a missing key, a wrong type. The raise keyword is different: it lets your own code declare an exception on purpose, to enforce a rule that Python itself has no way of knowing about.

  def set_vow(hair_status):
      if hair_status == "bound":
          raise ValueError("This vow cannot be silenced until it is fulfilled")

Nothing about hair_status == "bound" is invalid to Python — strings compare to strings fine. The exception exists because your code has decided this condition matters and deliberately says so.`,
